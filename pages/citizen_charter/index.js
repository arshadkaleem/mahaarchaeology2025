...
const Index = ({ citizenData }) => {
  ...
  return (
    <div>
      ...
      <SubPageHeader title={t("CitizenCharter")} />
      <div className="w-full px-2 py-10 sm:px-0 sm:w-11/12 mx-auto">
        <p className="text-justify text-lg pb-10 text-slate-700">
          {t("CitizenCharter-p")}
        </p>
...
export async function getServerSideProps({ locale }) {
  const { data } = await AllApi.titleDocAPI("Citizen Charter");
  const citizenData = data.actsAndLegislationCollection.items;
