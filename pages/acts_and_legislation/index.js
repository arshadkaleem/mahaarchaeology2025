...
const Index = ({ actsData }) => {
  ...
  return (
    <div>
      ...
      <SubPageHeader title={t("ActsHeading")} />
      <div className="w-full px-2 py-10 sm:px-0 sm:w-11/12 mx-auto">
        <p className="text-left  py-2 font-semibold">{t("ActsSubHeading")}</p>
        <div className="flex   ">
...
export async function getServerSideProps({ locale }) {
  const { data } = await AllApi.titleDocAPI("Acts and Legislation");
  const actsData = data.actsAndLegislationCollection.items;
